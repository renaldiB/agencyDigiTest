...
/***********************************************************************************************/
        ...
        var aiMoveAction = (level) => {
            const thinkingTime = 1000; // AI thinking time
        ...
        const performMove = (moveResult) => {
            ...
            const removePieceAtIndex = (index) => {
              scene.remove(this.boardPiecesArray[index].mesh);
              this.boardPiecesArray.splice(index, 1);
            };
            ...
            const piece = this.boardPiecesArray[fromIndex].pieceType;
            if (piece === "K" || piece === "k") {
              const castlingMove = castlingMoves[currentTurn][piece];
              if (
                to === castlingMove.target &&
                this.props.game.board.configuration.castling[castlingMove.long]
              ) {
                const matrixIndex = getMatrixIndexFromFen(castlingMove.rookFrom);
                const rook = this.boardPiecesArray.find(
                  (item) =>
                    item.rowIndex === matrixIndex.rowIndex &&
                    item.colIndex === matrixIndex.colIndex
                );
                const targetIndex = getMatrixIndexFromFen(castlingMove.rookTo);
                movePiece(rook, targetIndex.rowIndex, targetIndex.colIndex);
              } else if (
                to === castlingMove.target &&
                this.props.game.board.configuration.castling[castlingMove.short]
              ) {
                const matrixIndex = getMatrixIndexFromFen(castlingMove.rookFrom);
                const rook = this.boardPiecesArray.find(
                  (item) =>
    ...
    handlePerformMove(params) {
        ...
        const removePieceAtIndex = (index) => {
            this.scene.remove(this.boardPiecesArray[index].mesh);
            this.boardPiecesArray.splice(index, 1);
        };
        ...
        if (fromIndex !== -1) {
            // enpassant case
            if ((this.boardPiecesArray[fromIndex].pieceType.toLowerCase() === 'p') && to === enPassant) {
                const targetMatrixIndex = { ...toMatrixIndex };
                targetMatrixIndex.rowIndex += (this.currentTurn === 'white' ? -1 : 1);
                ...
                if (targetIndex !== -1) {
                    removePieceAtIndex(targetIndex);
                }
            }
        ...
        const castlingMoves = {
            whiteLong: { from: 'A1', to: 'D1' },
            whiteShort: { from: 'H1', to: 'F1' },
            blackLong: { from: 'A8', to: 'D8' },
            blackShort: { from: 'H8', to: 'F8' }
        };
    
        for (const [key, value] of Object.entries(castlingMoves)) {
            if (castling[key]) {
                const matrixIndex = getMatrixIndexFromFen(value.from);
                const rook = this.boardPiecesArray.find((item) => item.rowIndex === matrixIndex.rowIndex && item.colIndex === matrixIndex.colIndex);
                if (rook) {
                    const targetIndex = getMatrixIndexFromFen(value.to);
                    this.movePiece(rook, targetIndex.rowIndex, targetIndex.colIndex);
                }
            }
        }
